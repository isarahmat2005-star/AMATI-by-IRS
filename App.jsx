...
const CopyIcon = ({ className }) => <svg xmlns="http://www.w3.org/2000/svg" className={className || "w-3 h-3"} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>;
...
const Wand2Icon = ({ className }) => <svg xmlns="http://www.w3.org/2000/svg" className={className || "w-4 h-4"} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m21.64 3.64-1.28-1.28a1.21 1.21 0 0 0-1.72 0L2.36 18.64a1.21 1.21 0 0 0 0 1.72l1.28 1.28a1.2 1.2 0 0 0 1.72 0L21.64 5.36a1.2 1.2 0 0 0 0-1.72Z"/><path d="m14 7 3 3"/><path d="M5 6v4"/><path d="M19 14v4"/><path d="M10 2v2"/><path d="M7 8H3"/><path d="M21 16h-4"/><path d="M11 3H9"/></svg>;
const AlertTriangleIcon = ({ className }) => <svg xmlns="http://www.w3.org/2000/svg" className={className || "w-6 h-6"} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>;
const CodeIcon = ({ className }) => <svg className={className || "w-4 h-4"} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="16 18 22 12 16 6"></polyline><polyline points="8 6 2 12 8 18"></polyline></svg>;
...
const CheckCircleIcon = ({ className }) => <svg xmlns="http://www.w3.org/2000/svg" className={className || "w-3 h-3"} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>;
const XCircleIcon = ({ className }) => <svg xmlns="http://www.w3.org/2000/svg" className={className || "w-4 h-4"} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>;
const TrashIcon = ({ className }) => <svg xmlns="http://www.w3.org/2000/svg" className={className || "w-3 h-3"} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>;
...
const DownloadIcon = ({ className }) => <svg xmlns="http://www.w3.org/2000/svg" className={className || "w-3 h-3"} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>;
...
const EyeIcon = ({ className }) => <svg className={className || "w-3 h-3"} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>;
const UploadCloudIcon = ({ className }) => <svg xmlns="http://www.w3.org/2000/svg" className={className || "w-4 h-4"} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 14.899A7 7 0 1 1 15.71 8h1.79a4.5 4.5 0 0 1 2.5 8.242"/><path d="M12 12v9"/><path d="m16 16-4-4-4 4"/></svg>;
...
const wrapSvgAsHtml = (svgCode, resolution = '1920x1080', duration = 10, viewMode = 'preview') => {
    const isThumb = viewMode === 'thumbnail';
    const isPreview = viewMode === 'preview';

    let processedSvg = svgCode;
    processedSvg = processedSvg.replace(/preserveAspectRatio="[^"]*"/gi, '');
    processedSvg = processedSvg.replace(/<svg\s/i, '<svg preserveAspectRatio="xMidYMid slice" ');
...
<body>
    <div class="svg-wrapper">${processedSvg}</div>
    ${playerHtml}
    ...
    <script>
        ...
        const setAnimationState = (state) => {
            ...
            if(!svg) return;
            if (state === 'paused' && typeof svg.pauseAnimations === 'function') svg.pauseAnimations();
            else if (state === 'running' && typeof svg.unpauseAnimations === 'function') svg.unpauseAnimations();
            
            svg.style.animationPlayState = state;
            const els = svg.querySelectorAll('*');
            for(let i=0; i<els.length; i++) els[i].style.animationPlayState = state;
        };
        ...
        window.addEventListener('DOMContentLoaded', () => {
            if (!isPlaying) {
                ...
                setTimeout(() => setAnimationState('paused'), 50);
            }
        });
        ...
        const updateTimeFromEvent = (clientX) => {
            ...
            const svg = document.querySelector('svg');
            if(svg && typeof svg.setCurrentTime === 'function') svg.setCurrentTime(targetTime);
...
// --- APLIKASI UTAMA (RENDER ENGINE SAJA) ---
export default function App() {
    ...
    const handleRenderUpload = async (e) => {
        ...
        const newFiles = await Promise.all(txtFiles.map(async (file) => {
            ...
            const vbMatch = content.match(/viewBox="[^"]*?\s(\d+(?:\.\d+)?)\s+(\d+(?:\.\d+)?)"/i);
            if(vbMatch) {
            ...
            // Deteksi Durasi
            let dur = 10;
            const durMatches = content.match(/(\d+(?:\.\d+)?)s/gi);
            if(durMatches) {
                let maxDur = 0;
                durMatches.forEach(m => {
                    let val = parseFloat(m);
                    if(val > maxDur && val <= 300) maxDur = val;
                });
    ...
    // --- PROSES RENDER (SIMULASI PROGRESS BAR UNTUK UI) ---
    const startRenderAction = async (isResume = false) => {
            ...
            uploadedRenderFiles.forEach(file => {
                for(let i=0; i<qty; i++) {
                    newTasks.push({
                        ...
                        framesRendered: 0,
                        totalFrames: file.duration * renderFps // Hitung jumlah frame berdasar pilihan FPS
                    });
                }
            });
            setCards(prev => [...newTasks, ...prev.map(f => f.status === 'failed' ? { ...f, status: 'pending', error: null, framesRendered: 0 } : f)]);
            setCurrentPage(1); 
        }
        ...
        while (!isPausedRef.current) {
            ...
            setCards(prev => prev.map(f => f.id === taskToProcess.id ? { ...f, status: 'processing', framesRendered: 0 } : f));
            
            try {
                // --- SIMULASI MOCKUP PROSES RENDER ---
                // Di Vercel nanti, ganti loop ini dengan logika pemanggilan FFMPEG WebAssembly aslinya.
                // // AREA EKSEKUSI FFMPEG
                const totalF = taskToProcess.totalFrames;
                for (let frame = 1; frame <= totalF; frame++) {
                    if (isPausedRef.current) break; // Berhenti jika tombol pause ditekan
                    
                    // Update progress UI (Anggap 1 frame butuh ~30ms proses rendering)
                    await new Promise(r => setTimeout(r, 20)); 
                    
                    setCards(prev => prev.map(f => f.id === taskToProcess.id ? { ...f, framesRendered: frame } : f));
                }
                
                if (!isPausedRef.current) {
                    setCards(prev => prev.map(f => f.id === taskToProcess.id ? { ...f, status: 'done', code: taskToProcess.code /* Dummy blob nanti */ } : f));
                }
                
            } catch (error) {
                setCards(prev => prev.map(f => f.id === taskToProcess.id ? { ...f, status: 'failed', error: error.message } : f));
            }
        }
    ...
    const handlePauseResumeRender = () => {
        if ((isRendering || countProcessing > 0) && !isPaused) { 
            ...
            // Biarkan status tetap processing agar bisa di-resume dari frame terakhir jika menggunakan FFmpeg real
            // Untuk simulasi ini kita kembalikan ke pending agar rapi
    ...
    // EKSPOR MP4 (Simulasi Zip File MP4)
    const handleDownloadZipMP4 = async () => {
        const doneCards = cards.filter(f => f.status === 'done');
        ...
        try {
            ...
            for (let i = 0; i < doneCards.length; i++) {
                const img = doneCards[i];
                // Dalam skenario asli, img.code berisi data Blob MP4, bukan teks.
                zip.file(`${img.title || `amati_render_${img.id}`}.txt`, img.code); // Ganti ekstensi jadi .mp4 nanti
            }
            const content = await zip.generateAsync({ type: 'blob' });
            const zipUrl = URL.createObjectURL(content);
            const link = document.createElement('a');
            link.href = zipUrl; link.download = `${zipFilename.trim() || 'AMATI-Motion-Video'}.zip`;
    ...
    const handleDownloadSingleMP4 = (cardData) => {
        // Simulasi Download MP4 Tunggal
        const blob = new Blob([cardData.code], { type: 'text/plain' }); // Nanti ganti 'video/mp4'
        const url = URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `${cardData.title}.txt`; // Nanti ganti .mp4
                        ...
                        <div className="flex-1 flex flex-col overflow-y-visible lg:overflow-y-auto overflow-x-hidden custom-scroll">
                            <div className="p-4 flex flex-col gap-4">
                                ...
                                <div className="flex w-full">
                                    ...
                                    <a href="#" className="w-full flex items-center justify-between bg-[#0891B3] hover:bg-[#06738F] text-white font-bold py-3 px-4 rounded-lg transition shadow-md text-xs tracking-wide hover:-translate-y-0.5 duration-200 uppercase">
                                ...
                                <div className="bg-white p-4 rounded-lg shadow-sm border border-[#0891B3]/30 flex flex-col text-left">
                                    ...
                                    <div className="mb-4 shrink-0 flex flex-col">
                                        <div className="flex justify-between items-end mb-1">
                                            <label className="block text-[11px] font-bold text-slate-600">Daftar Kode SVG (.TXT)</label>
                                        </div>
                        ...
                        <div className="shrink-0 p-4 bg-slate-50 border-t border-slate-200 flex flex-col gap-4 z-10">
                            <div className="bg-white rounded-lg border border-slate-200 shadow-sm transition-all overflow-hidden">
                                ...
                                <div className="p-2 bg-white flex items-center justify-between gap-3">
                                    <button onClick={() => setClearAllConfirm(true)} disabled={isRendering && !isPaused || cards.length === 0} className={`flex-1 flex items-center justify-center gap-2 py-1.5 text-xs font-bold uppercase tracking-wide rounded border transition-colors ${cards.length > 0 && (!isRendering || isPaused) ? 'bg-red-50 text-red-600 border-red-200 hover:bg-red-100' : 'bg-gray-50 text-gray-400 border-gray-200 cursor-not-allowed opacity-50'}`}>
                                        <TrashIcon className="w-3.5 h-3.5"/> CLEAR ALL KARTU
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="flex gap-1.5 h-10">
                                ...
                                <button onClick={handleDownloadZipMP4} disabled={!isZipActive || isZipping} className={`flex-1 text-xs font-bold rounded-lg border shadow transition-colors flex items-center justify-center gap-2 uppercase tracking-wide truncate ${isZipActive ? 'bg-green-600 hover:bg-green-700 text-white border-green-700 hover:-translate-y-0.5' : 'bg-slate-100 text-slate-400 border-slate-200 cursor-not-allowed opacity-80'}`}>
                                    {isZipping ? <CustomSpinner className="w-4 h-4 text-white" /> : <DownloadIcon className="w-3.5 h-3.5"/>}
                            ...
                            {cards.length > 0 ? (
                                <div className="grid gap-4 items-start" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))' }}>
                                    {paginatedCards.map(f => {
                                        ...
                                        const progressPercent = f.totalFrames ? Math.floor((f.framesRendered / f.totalFrames) * 100) : 0;
                                        ...
                                        <div key={f.id} className={`bg-white hover:shadow-md rounded-lg shadow-sm border flex flex-col transition-all duration-300 ${f.status === 'processing' ? 'border-amber-400 ring-2 ring-amber-400/20' : f.status === 'failed' ? 'border-red-300' : 'border-slate-200'}`}>
                                            ...
                                            <div className="grid grid-cols-2 gap-2 p-2 bg-[#0891B3]/5 border-b border-[#0891B3]/10 rounded-t-lg shrink-0">
                                                ...
                                                {f.status === 'done' ? (
                                                    <button onClick={() => handleDownloadSingleMP4(f)} className="flex flex-row items-center justify-center gap-1.5 py-1.5 rounded border bg-emerald-500 border-emerald-600 text-white hover:bg-emerald-600 transition-colors shadow-sm">
                                                        <DownloadIcon className="w-3.5 h-3.5"/> <span className="text-[10px] font-bold uppercase tracking-tight truncate">Download MP4</span>
                                                    </button>
                                                ) : (
                                                    <button onClick={() => setFileToDelete(f.id)} disabled={f.status === 'processing'} className="flex flex-row items-center justify-center gap-1.5 py-1.5 rounded border bg-white border-red-200 text-red-500 hover:bg-red-50 transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
                                                        <TrashIcon className="w-3.5 h-3.5"/> <span className="text-[10px] font-bold uppercase tracking-tight truncate">Hapus</span>
                                                    </button>
                                                )}
                                            </div>
                                            ...
                                            {/* UI KARTU RENDER 50:50 (KIRI ANIMASI, KANAN HITUNGAN FRAME) */}
                                            <div className="p-2 flex gap-2 h-[150px] bg-white rounded-b-lg relative">
                                                
                                                {/* BAGIAN KIRI: Visual/Animasi */}
                                                <div className="flex-1 rounded-lg overflow-hidden bg-slate-800 relative flex items-center justify-center border border-slate-300 shadow-inner">
                                                    {f.status === 'done' ? (
                                                        <div className="flex flex-col items-center justify-center text-emerald-400">
                                                            <CheckCircleIcon className="w-10 h-10 mb-2"/>
                                                            <span className="text-[10px] font-bold uppercase tracking-widest">Render Selesai</span>
                                                        </div>
                                                    ) : f.status === 'failed' ? (
                                                        <div className="flex flex-col items-center justify-center text-red-400">
                                                            <AlertTriangleIcon className="w-8 h-8 mx-auto mb-2" />
                                                            <span className="text-[10px] font-bold uppercase tracking-widest text-center">Gagal Render</span>
                                                        </div>
                                                    ) : f.status === 'processing' ? (
                                                        <div className="flex flex-col items-center justify-center text-amber-400">
                                                            <CustomSpinner className="w-8 h-8 mb-2 text-amber-400" />
                                                            <span className="text-[10px] font-bold uppercase tracking-widest">{progressPercent}%</span>
                                                        </div>
                                                    ) : (
                                                ...
                                                <div className="flex-1 border border-slate-200 rounded-lg bg-slate-50 flex flex-col justify-center items-center p-3 relative overflow-hidden">
                                                    ...
                                                    <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1">Frame Rendered</span>
                                ...
                                <div className="flex-1 flex flex-col items-center justify-center text-center w-full h-full min-h-[50vh]">
                                    ...
                                    <p className="text-slate-500 text-sm max-w-md">Silakan upload kode animasi (.TXT) Anda di panel kiri untuk mulai merender ke format MP4.</p>
                                </div>
                            )}
                        </div>
                    </section>
                        ...
                        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/80 p-4 md:p-8 backdrop-blur-sm transition-opacity" onClick={() => setPreviewModal(null)}>
                            ...
                            >
                                ...
                                <div className="w-full" style={{ paddingBottom: `calc(${1 / aspect * 100}% + 56px)`, position: 'relative' }}>
                                    <iframe 
                                        srcDoc={wrapSvgAsHtml(previewModal.code, resStr, previewModal.duration, 'preview')} 
                ...
                {fileToDelete && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
                        <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-sm flex flex-col items-center text-center">
                            <div className="bg-red-100 p-3 rounded-full mb-3"><AlertTriangleIcon className="w-6 h-6 text-red-600"/></div>
