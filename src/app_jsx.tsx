...
const HUGGING_FACE_URLS = [
    "https://isaproject-render-mp4-1.hf.space",
...
<style>
    ...
    .svg-wrapper { flex: 1; display: flex; align-items: center; justify-content: center; width: 100%; overflow: hidden; ${isPreview ? 'background: #f8fafc; border-radius: 8px; border: 1px solid #cbd5e1;' : 'background: #000;'} }
...
<body>
    ...
    <script>
        ...
        const duration = ${duration}; let isPlaying = ${isThumb ? 'false' : 'true'}; let startTime = performance.now(); let accumulatedTime = 0;
        const btn = document.getElementById('btnPlayPause'); const iconPlay = document.getElementById('iconPlay'); const iconPause = document.getElementById('iconPause'); const fill = document.getElementById('fill'); const timeDisp = document.getElementById('timeDisp'); const track = document.getElementById('track');
        const svgEl = document.getElementById('canvas'); const res = '${resolution}'.split('x'); const width = parseInt(res[0]) || 1920; const height = parseInt(res[1]) || 1080; svgEl.setAttribute('viewBox', \`0 0 \${width} \${height}\`);
        if (typeof create === 'function') { try { create(svgEl, width, height); } catch(e) { console.error("Error dalam fungsi create():", e); } }

        window.addEventListener('DOMContentLoaded', () => { if (!isPlaying) { if (iconPause) iconPause.style.display = 'none'; if (iconPlay) iconPlay.style.display = 'block'; if (typeof update === 'function') update(0, svgEl, width, height); } });

        let isDragging = false;
        const updateTimeFromEvent = (clientX) => { const rect = track.getBoundingClientRect(); const percent = Math.max(0, Math.min(100, ((clientX - rect.left) / rect.width) * 100)); const targetTime = (percent / 100) * duration; accumulatedTime = targetTime; if (typeof update === 'function') update(targetTime, svgEl, width, height); if(fill && timeDisp) { fill.style.width = percent + '%'; timeDisp.innerText = targetTime.toFixed(1) + 's'; } };

        if (track) { track.addEventListener('mousedown', (e) => { isDragging = true; updateTimeFromEvent(e.clientX); }); window.addEventListener('mousemove', (e) => { if(isDragging) updateTimeFromEvent(e.clientX); }); window.addEventListener('mouseup', () => { if(isDragging) { isDragging = false; startTime = performance.now(); } }); }

        const runLoop = () => { if(!isPlaying) { requestAnimationFrame(runLoop); return; } const now = performance.now(); const elapsed = (now - startTime) / 1000; let current = (accumulatedTime + elapsed) % duration; if (typeof update === 'function') { try { update(current, svgEl, width, height); } catch(e) {} } if (fill && timeDisp && !isDragging) { fill.style.width = ((current / duration) * 100) + '%'; timeDisp.innerText = current.toFixed(1) + 's'; } requestAnimationFrame(runLoop); }; requestAnimationFrame(runLoop);

        if (btn && track) { btn.addEventListener('click', () => { if(isPlaying) { isPlaying = false; iconPause.style.display = 'none'; iconPlay.style.display = 'block'; accumulatedTime = (accumulatedTime + (performance.now() - startTime) / 1000) % duration; } else { isPlaying = true; iconPlay.style.display = 'none'; iconPause.style.display = 'block'; startTime = performance.now(); } }); }
    </script>
</body>
</html>`;
};
...
export default function App() {
    ...
    const handleRenderUpload = async (e) => {
        ...
        const newFiles = await Promise.all(txtFiles.map(async (file) => {
            ...
            return { id: Date.now() + Math.random(), name: file.name, content, resolution: `${width}x${height}`, ratio: ratioStr, duration: dur };
        }));
    ...
    // --- PROSES RENDER NATIVE BROWSER MENGGUNAKAN API HUGGING FACE ---
    const startRenderAction = async (isResume = false) => {
        if (isRenderingRef.current) return;
        
        // Pengecekan URL Hugging Face
        if (HUGGING_FACE_URLS.length === 0 || HUGGING_FACE_URLS[0] === "URL_HUGGING_FACE_ANDA_DISINI") {
             setGlobalMessage({ title: "URL Belum Disetting", type: "error", text: "Silakan masukkan URL Hugging Face di dalam kode App.jsx baris atas." });
             return;
        }
        ...
        try {
            ...
            while (!isPausedRef.current) {
                ...
                try {
                    // Load Balance: Pilih URL secara acak dari array
                    const targetUrl = HUGGING_FACE_URLS[Math.floor(Math.random() * HUGGING_FACE_URLS.length)];
                    
                    const response = await fetch(`${targetUrl.replace(/\/$/, '')}/api/predict`, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ 
                            data: [task.code, task.resolution, task.duration, renderFps] 
                        })
                    });
                    ...
                    if (!isPausedRef.current) {
                        if (finalUrl) {
                            ...
                            throw new Error("Format balasan dari server HF tidak dikenali.");
                        }
                    } else {
                        setCards(prev => prev.map(c => c.id === task.id ? { ...c, status: 'pending', framesRendered: 0 } : c));
                    }
                } catch (error) {
                    console.error("Gagal Render API HF:", error);
                    setCards(prev => prev.map(f => f.id === task.id ? { ...f, status: 'failed', error: "Browser atau Mesin HF gagal memproses kode animasi." } : f));
                }
            }
        } catch (globalErr) {
            setGlobalMessage({ title: "Error Modul", type: "error", text: "Koneksi ke server gagal. Pastikan URL HF aktif." });
        }
    ...
    return (
        <>
            ...
            {/* 
              PERBAIKAN HP LAYOUT: 
              Kita membuang "lg:h-screen lg:overflow-hidden" dari div terluar ini 
              agar saat dipakai di HP atau layar kecil, halamannya akan memanjang natural 
              (tidak macet scrollnya).
            */}
            <div className="min-h-screen bg-slate-100 text-slate-900 flex flex-col">
                ...
                <main className="w-full flex-1 flex flex-col lg:flex-row relative">
                    <aside className="w-full lg:w-[380px] bg-slate-50 lg:border-r border-slate-200 flex flex-col z-20 shrink-0">
                        
                        <div className="flex-1 flex flex-col lg:h-[calc(100vh-56px)] lg:sticky lg:top-14">
                            <div className="p-4 flex flex-col gap-4 flex-1 lg:overflow-y-auto lg:overflow-x-hidden custom-scroll">
                            ...
                            {/* 
                                KOTAK STATUS RENDER BAWAH
                                Saat di HP, kotak ini akan jadi footer menempel di bawah / tetap ter-render wajar
                            */}
                            <div className="shrink-0 p-4 bg-slate-50 border-t border-slate-200 flex flex-col gap-4 z-10 sticky bottom-0 lg:static lg:bottom-auto">
                    ...
                    <section className="flex-1 flex flex-col relative lg:h-screen lg:overflow-hidden">
                        <div className="bg-white border-b border-slate-200 p-3 flex justify-between items-center shrink-0 shadow-sm z-10 sticky top-[56px] lg:top-0">
                        ...
                        <div className="flex-1 p-4 custom-scroll pb-20 lg:pb-4 lg:overflow-y-auto">
                            {cards.length > 0 ? (
                                <div className="grid gap-4 items-start" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))' }}>
                                    {paginatedCards.map(f => {
                                        const progressPercent = f.status === 'processing' ? 50 : (f.status === 'done' ? 100 : 0);
                                                ...
                                                <div className="flex-1 rounded-lg overflow-hidden bg-slate-800 relative flex items-center justify-center border border-slate-300 shadow-inner group cursor-pointer" onClick={() => f.status === 'done' && setPreviewModal(f)}>
                                                    {f.status === 'done' || f.status === 'pending' ? (
                                                        ...
                                                        <div className="flex flex-col items-center justify-center text-amber-400">
                                                            ...
                                                            <span className="text-[10px] font-bold uppercase tracking-widest text-center mt-2">Server HF<br/>Merender...</span>
                                                        </div>
                                                    ) : null}
                                                </div>
