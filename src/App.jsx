...
export default function App() {
    ...
    // --- PROSES RENDER NATIVE BROWSER (WebCodecs + mp4-muxer) ---
    const startRenderAction = async (isResume = false) => {
        ...
        try {
            // Impor dinamis library MP4 Muxer yang sangat ringan (langsung jalan di browser)
            const Mp4Muxer = await import('https://esm.sh/mp4-muxer@3.0.0');
            ...
            while (!isPausedRef.current) {
                ...
                try {
                    const width = parseInt(task.resolution.split('x')[0]);
                    const height = parseInt(task.resolution.split('x')[1]);
                    const totalF = task.totalFrames;

                    // 1. Siapkan Dapur Rahasia (Elemen SVG tersembunyi)
                    const container = document.createElement('div');
                    container.style.position = 'absolute'; container.style.opacity = '0'; container.style.pointerEvents = 'none';
                    ...
                    // 2. Suntikkan kode user ke dapur rahasia
                    const createFn = new Function('svg', 'width', 'height', task.code + '\nreturn typeof create === "function" ? create : null;')()(svgEl, width, height);
                    const updateFn = new Function('time', 'svg', 'width', 'height', task.code + '\nreturn typeof update === "function" ? update : null;')();

                    // 3. Siapkan Kanvas dan Kamera Perekam (WebCodecs)
                    ...
                    videoEncoder.configure({
                        codec: 'avc1.42001f', // Codec H.264 standar yang aman di semua perangkat
                        width: width, 
                        height: height,
                        bitrate: renderBitrate * 1000000,
                        framerate: renderFps
                    });

                    // 4. Proses Rekam Frame demi Frame
                    for (let f = 1; f <= totalF; f++) {
                        if (isPausedRef.current) break; 
                        
                        const time = (f / renderFps);
                        if (updateFn) updateFn(time, svgEl, width, height);

                        // Potret SVG jadi XML Teks
                        let svgData = new XMLSerializer().serializeToString(svgEl);
                        if(!svgData.includes('xmlns=')) svgData = svgData.replace('<svg', '<svg xmlns="http://www.w3.org/2000/svg"');
                        
                        // Ubah XML jadi Gambar, lalu gambar ke Kanvas
                        const img = new Image();
                        ...
                        await new Promise((resolve) => {
                            img.onload = () => {
                                ctx.fillStyle = '#000000'; // Default warna layar hitam
                        ...
                        // Masukkan gambar kanvas ke mesin VideoEncoder
                        const frame = new VideoFrame(canvas, { timestamp: (f * 1000000) / renderFps });
                        videoEncoder.encode(frame);
                        frame.close();
                        
                        // Update Progress UI
                        setCards(prev => prev.map(c => c.id === task.id ? { ...c, framesRendered: f } : c));
                    }
                    
                    if (!isPausedRef.current) {
                        // 5. Jahit Semua Frame menjadi MP4 Utuh
                        ...
                        document.body.removeChild(container); // Bersihkan dapur rahasia
                        
                        setCards(prev => prev.map(c => c.id === task.id ? { ...c, status: 'done', outputUrl: finalUrl } : c));
                    } else {
                        document.body.removeChild(container);
                        // Jika ter-pause, batalkan hasil render setengah jalan
                        setCards(prev => prev.map(c => c.id === task.id ? { ...c, status: 'pending', framesRendered: 0 } : c));
                    }
                    
                } catch (error) {
                    console.error(error);
                    setCards(prev => prev.map(f => f.id === task.id ? { ...f, status: 'failed', error: "Browser tidak mendukung WebCodecs atau ada error di kode animasi." } : f));
                }
            }
        } catch (globalErr) {
            setGlobalMessage({ title: "Error Modul", type: "error", text: "Gagal memuat mesin MP4 Muxer. Pastikan koneksi internet stabil." });
        }
                        ...
                        <div className="flex-1 flex flex-col overflow-y-visible lg:overflow-y-auto overflow-x-hidden custom-scroll">
                            <div className="p-4 flex flex-col gap-4">
                                ...
                                <div className="flex w-full">
                                    <a href="https://share.gemini.google/2CfkabvOXUO7" className="w-full flex items-center justify-between bg-[#0891B3] hover:bg-[#06738F] text-white font-bold py-3 px-4 rounded-lg transition shadow-md text-xs tracking-wide hover:-translate-y-0.5 duration-200 uppercase">
